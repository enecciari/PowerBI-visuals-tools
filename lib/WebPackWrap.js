...
class WebPackGenerator {
    ...
    static loadWebpackMerge() {
        try {
            let mergePath = require.resolve('webpack-merge', {
                paths: [process.cwd()]
            });
            return require(mergePath);
        } catch (ex) {
            return null;
        }
    }
    ...
    async prepareWebPackConfig(visualPackage, options, tsconfig) {
        ...
        const merge = loadWebpackMerge();
        if (merge !== null) {
            const localWebpackConfigPath = path.join(process.cwd(), 'webpack.config');
            if (fs.existsSync(localWebpackConfigPath)) {
